 ...
 */
public class GatewayAwareUrlAuthenticationHandler implements AuthenticationHandler, InitializingBean {

	public static final String GATEWAY_ATTEMPT = "GATEWAY_ATTEMPT";
	...
	public void afterPropertiesSet() throws Exception {
		Assert.notNull(this.authenticationFailureHandler, "No authenticationFailureHandler to delegate");
		Assert.notNull(this.authenticationSuccessHandler, "No authenticationSuccessHandler to delegate");
	}

	@Override
	public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
		if (isGatewayRequest(request)) {
			processGatewayRequestFailure(request, response, exception);
			return;
		}
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
		request.getSession().removeAttribute(GATEWAY_ATTEMPT);
		this.authenticationSuccessHandler.onAuthenticationSuccess(request, response, authentication);
	}

	protected void processGatewayRequestFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException authenticationException) throws IOException, ServletException {
		this.logger.debug("gateway request authentication failure, should be ignored.");
		SavedRequest cachedRequest = this.requestCache.getRequest(request, response);
		if (cachedRequest != null) {
			String redirectUrl = cachedRequest.getRedirectUrl();
	...
	protected boolean isGatewayRequest(HttpServletRequest request) {
		boolean hasAttribute = request.getSession().getAttribute(GATEWAY_ATTEMPT) != null;
		if (hasAttribute) {
			request.getSession().removeAttribute(GATEWAY_ATTEMPT);
		}
		return hasAttribute;
